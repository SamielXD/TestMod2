...
public class TestMod extends Mod {
    ...
    void buildSimpleModEntry(Table table, ModInfo mod) {
        table.button(b -> {
            ...
            titleRow.left();
            titleRow.add("[accent]" + mod.name).padRight(5f);
            ...
            if (mod.modType != null) {
                ...
                if (badge == null) {
                    if (mod.modType.equals("java")) {
                        titleRow.add("[#b07219][[JAVA]").padLeft(5f);
                    } else if (mod.modType.equals("javascript")) {
                        titleRow.add("[#f1e05a][[JS]").padLeft(5f);
                    }
                } else {
                    badge.setScaling(Scaling.fit);
                    titleRow.add(badge).size(20f, 14f).padLeft(5f);
                }
            }
            
            info.add(titleRow).row();
    ...
    void showModDetails(ModInfo mod) {
        ...
        if (mod.modType != null) {
            ...
            if (badge == null) {
                if (mod.modType.equals("java")) {
                    titleTable.add("[#b07219][[JAVA]");
                } else if (mod.modType.equals("javascript")) {
                    titleTable.add("[#f1e05a][[JavaScript]");
                }
            } else {
                badge.setScaling(Scaling.fit);
                titleTable.add(badge).size(32f, 22f);
            }
        }
        
        content.add(titleTable).center().row();
        ...
        fetchModStats(mod, stats -> {
            ...
            if (mod.modType != null) {
                String typeStr = mod.modType.equals("java") ? "[#b07219]Java Mod" : "[#f1e05a]JavaScript Mod";
                statsTable.add(typeStr).row();
            }
        });
    }
    ...
    Seq<ModInfo> parseModList(String json) {
        ...
        try {
            ...
            for (String modStr : modObjects) {
                ...
                // Detect mod type (java or javascript)
    ...
    String detectModType(String modJson) {
        // Check if the mod has a "java" field set to true
        if (modJson.contains("\"java\":true") || modJson.contains("\"java\": true")) {
            return "java";
        }
        // Otherwise assume it's JavaScript
        return "javascript";
    }
