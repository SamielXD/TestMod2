...
public class TooltipsPlusMod extends Mod {
    ...
    ObjectMap<Building, Float> buildingHealthCache = new ObjectMap<>();
    ObjectMap<Unit, Float> unitHealthCache = new ObjectMap<>();
    ...
    String makeProgressBar(float current, float max, int width) {
        ...
        try {
            Vars.ui.settings.addCategory("Tooltips+", Icon.book, table -> {
                ...
                table.add("[lightgray]Keys: T=Toggle | P=Pin | R=Range | H=Health").colspan(2).left().padTop(4f).row();
                
                table.add("[accent]" + repeat("═", 13) + " Visual UI " + repeat("═", 13)).center().colspan(2).padTop(12f).padBottom(8f).row();
                
                table.check("Health Bars", showHealthBars, v -> {
                    showHealthBars = v;
                    saveSettings();
                }).colspan(2).left().row();
                
                table.check("Shield Stacks", showShieldStacks, v -> {
                    showShieldStacks = v;
                    saveSettings();
                }).colspan(2).left().row();
                
                table.check("Range Indicators", showRangeIndicators, v -> {
                    showRangeIndicators = v;
                    saveSettings();
                }).colspan(2).left().row();
                
                table.check("Effect Ranges", showEffectRanges, v -> {
                    showEffectRanges = v;
                    saveSettings();
                }).colspan(2).left().row();
                
                table.check("Animate Ranges", animateRanges, v -> {
                    animateRanges = v;
                    saveSettings();
                }).colspan(2).left().row();
                
                table.check("Show Unit Ranges", showUnitRanges, v -> {
                    showUnitRanges = v;
                    saveSettings();
                }).colspan(2).left().row();
                
                table.check("Show Team Ranges", showTeamRanges, v -> {
                    showTeamRanges = v;
                    saveSettings();
                }).colspan(2).left().row();
                
                table.add("Range Opacity: ").left();
                table.slider(0, 100, 5, (int)(rangeOpacity * 100), v -> {
                    rangeOpacity = v / 100f;
                    saveSettings();
                }).width(200f).row();
                
                table.add("[lightgray](" + (int)(rangeOpacity * 100) + "%)").colspan(2).left().padTop(-4f).row();
                
                table.add("Effect Opacity: ").left();
                table.slider(0, 100, 5, (int)(effectRangeOpacity * 100), v -> {
                    effectRangeOpacity = v / 100f;
                    saveSettings();
                }).width(200f).row();
                
                table.add("[lightgray](" + (int)(effectRangeOpacity * 100) + "%)").colspan(2).left().padTop(-4f).row();
